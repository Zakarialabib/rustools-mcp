...
impl ErrorIndexClient {
    ...
    pub async fn get_error_explanation(&self, code: &str) -> Result<String> {
        ...
        if !code.starts_with('E') || code.len() < 3 {
             return Err(anyhow!("Invalid error code format. Expected format like 'E0382'"));
        }
        ...
        if !response.status().is_success() {
            return Err(anyhow!("Error code '{}' not found (Status: {})", code, response.status()));
        }
        ...
        let main_selector = Selector::parse("main").map_err(|e| anyhow!("Selector error: {:?}", e))?; // fallback to body if needed?
        ...
        if let Some(main) = document.select(&main_selector).next() {
            ...
            Err(anyhow!("Could not find <main> content for error code {}", code))
        }
    }
}
