...
pub enum CacheKey {
    FindCrates { query: String, limit: Option<u32> },
    GetCrateOverview { crate_name: String, version: Option<String> },
    GetCrateModules { crate_name: String, version: Option<String>, limit: Option<usize> },
    GetSymbolDocs { crate_name: String, symbol_path: String, version: Option<String> },
    FetchRawDoc(DocsRsParams),
    ReadSourceFile { crate_name: String, path: String, version: Option<String>, start_line: Option<usize>, end_line: Option<usize> },
    GetCrateDependencies { crate_name: String, version: String, kind: Option<String> },
    GetCrateExamples { crate_name: String, version: Option<String>, limit: Option<usize> },
    AnalyzeFeatureFlags { crate_name: String, version: String },
    FindBySignature { crate_name: String, signature_pattern: String, version: Option<String> },
    FindTraitImplementors { crate_name: String, trait_path: String, version: Option<String>, limit: Option<usize> },
    GetContextBundle { crate_name: String, version: Option<String> },
}
...
impl DocsRsClient {
    ...
    async fn get_latest_version(&self, crate_name: &str) -> Result<String, DocsFetchError> {
        self.get_crate_metadata(crate_name).await.map(|info| info.max_version)
    }

    async fn get_crate_metadata(&self, crate_name: &str) -> Result<CrateInfo, DocsFetchError> {
        let url = format!("https://crates.io/api/v1/crates/{}", crate_name);
        let response = self.client.get(&url)
            .header("User-Agent", format!("rdoc-mcp/{}", env!("CARGO_PKG_VERSION")))
            .send()
            .await?;
    ...
    async fn resolve_version(&self, crate_name: &str, version: Option<String>) -> Result<String, DocsFetchError> {
    ...
    pub async fn search_crates(&self, query: String, limit: Option<u32>) -> Result<String, DocsFetchError> {
        ...
        let data: CratesIoSearchResponse = response.json().await
            .map_err(|e| DocsFetchError::ParseError(e.to_string()))?;
            
        let mut output = format!("# Search Results for '{}'\n\n", query);
        output.push_str("| Crate | Latest Version | Description |\n|-------|----------------|-------------|\n");
    ...
    pub async fn lookup_crate(&self, crate_name: String, version: Option<String>) -> Result<DocContent, DocsFetchError> {
        ...
        let mut content = self.extract_content(&html, &url)?;
    ...
    pub async fn extract_modules(&self, crate_name: String, version: Option<String>, _limit: Option<usize>) -> Result<DocContent, DocsFetchError> {
        // Just fetch the root page and parse modules
        self.lookup_crate(crate_name, version).await
    }

    pub async fn lookup_item(&self, crate_name: String, symbol_path: String, version: Option<String>) -> Result<DocContent, DocsFetchError> {
        // Handle fully qualified paths (e.g. "std::vec::Vec")
        let clean_path = if symbol_path.starts_with(&format!("{}::", crate_name)) {
            symbol_path.trim_start_matches(&format!("{}::", crate_name)).to_string()
        ...
        match self.lookup_item_internal(&crate_name, &parts, resolved_version).await {
    ...
    async fn lookup_item_external_windows(&self, parts: &[&str]) -> Result<String, DocsFetchError> {
        ...
        let item_types = ["struct", "enum", "trait", "fn", "macro", "type", "constant", "mod"];
            ...
            let url = if *item_type == "mod" {
                if module_path.is_empty() {
                 ...
                 if module_path.is_empty() {
                     ...
                     format!("{}/windows/{}/{}.{}.html", base, module_path, item_type, item_name)
                 }
            };
        ...
        Err(DocsFetchError::ItemNotFound("External windows docs not found".to_string()))
    }

    async fn lookup_item_internal(&self, crate_name: &str, parts: &[&str], version: String) -> Result<String, DocsFetchError> {
        let item_name = parts.last().unwrap();
        let module_path = parts[..parts.len()-1].join("/");
        ...
        for item_type in item_types.iter() {
            let url = if self.is_std_crate(crate_name) {
                ...
                if *item_type == "mod" {
                    if module_path.is_empty() {
                         ...
                         format!("{}/{}/{}/{}/index.html", base, crate_name, module_path, item_name)
                    }
                } else {
                     if module_path.is_empty() {
                         format!("{}/{}/{}.{}.html", base, crate_name, item_type, item_name)
                     } else {
                         format!("{}/{}/{}/{}.{}.html", base, crate_name, module_path, item_type, item_name)
                     }
                }
            } else {
                ...
                if *item_type == "mod" {
                    if module_path.is_empty() {
                        format!("{}/{}/{}/{}/{}/index.html", base, crate_name, version, crate_name, item_name)
                    } else {
                        format!("{}/{}/{}/{}/{}/{}/index.html", base, crate_name, version, crate_name, module_path, item_name)
                    }
                } else {
                    if module_path.is_empty() {
                        format!("{}/{}/{}/{}/{}.{}.html", base, crate_name, version, crate_name, item_type, item_name)
                    } else {
                        format!("{}/{}/{}/{}/{}/{}.{}.html", base, crate_name, version, crate_name, module_path, item_type, item_name)
                    }
                }
            };
            ...
            if response.status().is_success() {
                let html = response.text().await.map_err(|e| DocsFetchError::RequestError(e.to_string()))?;
                return self.extract_content(&html, &url);
            }
        }

        Err(last_error.unwrap_or_else(|| DocsFetchError::ItemNotFound(format!("Status: 404 Not Found"))))
    }

    pub async fn fetch_docs(&self, params: DocsRsParams) -> Result<DocContent, DocsFetchError> {
        let ver = self.resolve_version(&params.crate_name, Some(params.version)).await?;
        let url = format!("{}/{}/{}/{}", self.base_url, params.crate_name, ver, params.path);
        let response = self.client.get(&url).send().await?;
        if !response.status().is_success() {
            return Err(DocsFetchError::DocsNotFound);
        }
    ...
    pub async fn fetch_url(&self, url: &str) -> Result<String, DocsFetchError> {
        let response = self.client.get(url).send().await?;
        if !response.status().is_success() {
            return Err(DocsFetchError::RequestError(format!("Status: {}", response.status())));
        }
        let html = response.text().await?;
        self.extract_content(&html, url)
    }

    fn extract_content(&self, html: &str, url: &str) -> Result<String, DocsFetchError> {
    ...
    pub async fn analyze_feature_flags(&self, crate_name: String, version: String) -> Result<String, DocsFetchError> {
        let ver = self.resolve_version(&crate_name, Some(version)).await?;

        let url = format!("https://crates.io/api/v1/crates/{}/{}", crate_name, ver);
        let response = self.client.get(&url)
            .header("User-Agent", format!("rdoc-mcp/{} (contact@example.com)", env!("CARGO_PKG_VERSION")))
            .send()
            .await?;
        ...
        let api_response: CrateVersionResponse = response.json().await
            .map_err(|e| DocsFetchError::ParseError(e.to_string()))?;
        ...
        for (feature_name, enables) in features {
            ...
            let enables_str = if enables.is_empty() { "-".to_string() } else { enables.join(", ") };
            output.push_str(&format!("| **{}** | {} | {} |\n", feature_name, if is_default { "Yes" } else { "No" }, enables_str));
        }
        Ok(output)
    }

    pub async fn read_source_file(&self, crate_name: String, path: String, version: Option<String>, _start_line: Option<usize>, _end_line: Option<usize>) -> Result<DocContent, DocsFetchError> {
        ...
        let url = format!("{}/{}/{}/src/{}/{}.html", self.base_url, crate_name, ver, crate_safe, clean_path);
    ...
    pub async fn get_crate_dependencies(&self, crate_name: String, version: String, _kind: Option<String>) -> Result<String, DocsFetchError> {
         // Use crates.io dependencies API: https://crates.io/api/v1/crates/{crate_name}/{version}/dependencies
         let ver = self.resolve_version(&crate_name, Some(version)).await?;
         let url = format!("https://crates.io/api/v1/crates/{}/{}/dependencies", crate_name, ver);
         let response = self.client.get(&url)
            .header("User-Agent", format!("rdoc-mcp/{} (contact@example.com)", env!("CARGO_PKG_VERSION")))
            .send()
            .await?;
         ...
         if !response.status().is_success() {
             return Err(DocsFetchError::ParseError("Failed to fetch dependencies".to_string()));
         }
         // Parse JSON and format table (omitted for brevity, returning raw json)
         let text = response.text().await.map_err(|e| DocsFetchError::RequestError(e.to_string()))?;
         Ok(text)
    }

    pub async fn get_crate_examples(&self, crate_name: String, version: Option<String>, _limit: Option<usize>) -> Result<String, DocsFetchError> {
        let ver = self.resolve_version(&crate_name, version).await?;
        let crate_safe = crate_name.replace("-", "_");
        let url = format!("{}/{}/{}/src/{}/examples/", self.base_url, crate_name, ver, crate_safe);
        
        tracing::debug!("Checking for examples at: {}", url);

        let response = self.client.get(&url).send().await?;
        if response.status().is_success() {
             Ok(format!("Examples directory found at source root.\n\nPlease browse the examples here:\n{}", url))
        } else {
             // Fallback: Fetch metadata to get repo URL
             let repo_msg = match self.get_crate_metadata(&crate_name).await {
                 Ok(info) => {
                     match info.repository {
                         Some(repo) => format!("Please check the crate's repository for examples:\n{}", repo),
    ...
    pub async fn find_by_signature(&self, crate_name: String, pattern: String, version: Option<String>) -> Result<String, DocsFetchError> {
        let ver = self.resolve_version(&crate_name, version).await?;
        let base = self.get_base_url(&crate_name, &ver);
        
        let url = if self.is_std_crate(&crate_name) {
             format!("{}/{}/?search={}", base, crate_name, urlencoding::encode(&pattern))
        } else {
             format!("{}/{}/{}/{}/?search={}", base, crate_name, ver, crate_name, urlencoding::encode(&pattern))
        };
    ...
    pub async fn find_trait_implementors(&self, crate_name: String, trait_path: String, version: Option<String>, _limit: Option<usize>) -> Result<String, DocsFetchError> {
        ...
        match self.lookup_item(crate_name, trait_path, version).await {
            Ok(doc) => {
                ...
                if let Some(idx) = doc.content.find("# Implementors") {
                    ...
                    // or just a message.
                    Ok(format!("Could not isolate 'Implementors' section. Here is the full doc:\n\n{}", doc.content))
                }
            },
            Err(e) => Err(e)
        }
    ...
    pub async fn get_context_bundle(&self, crate_name: String, version: Option<String>) -> Result<String, DocsFetchError> {
         let ver = self.resolve_version(&crate_name, version.clone()).await?;
         
         // 1. Overview (README)
         let overview = match self.lookup_crate(crate_name.clone(), Some(ver.clone())).await {
             Ok(doc) => doc.content,
             Err(e) => format!("Failed to fetch overview: {}", e),
         };

         // 2. Feature Flags
         let features = match self.analyze_feature_flags(crate_name.clone(), ver.clone()).await {
             Ok(content) => content,
             Err(e) => format!("Failed to fetch feature flags: {}", e),
         };
         
         // 3. Dependencies
         let deps = match self.get_crate_dependencies(crate_name.clone(), ver.clone(), None).await {
             Ok(content) => content,
             Err(e) => format!("Failed to fetch dependencies: {}", e),
         };
         
         let bundle = format!(
             "# Context Bundle for {} {}\n\n## 1. Overview\n{}\n\n## 2. Feature Flags\n{}\n\n## 3. Dependencies\n{}", 
             crate_name, ver, overview, features, deps
         );
...
mod tests {
    ...
    async fn test_analyze_feature_flags_latest() {
         if std::env::var("RUN_REAL_NET_TESTS").is_err() { return; }
        let client = DocsRsClient::new();
        let result = client.analyze_feature_flags("serde".to_string(), "latest".to_string()).await;
        assert!(result.is_ok());
    }
}
